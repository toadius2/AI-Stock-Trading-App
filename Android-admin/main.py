...
class Admin(Screen):
    ...
    def direction_select(self, dir):
        self.direction = dir

    def stock_direction_select(self, dir):
        self.stock_dir = dir

    def all_direction_select(self, dir):
        self.all_direc = dir

    def publish_nonstock(self):
        part = Particle_publish()
        print(part.auth())
        ex = self.ids['exchange'].text.upper()
        curr = self.ids['currency'].text.upper()
        to_curr = self.ids['to_curr'].text.upper()
        amount = self.ids['amount'].text
        order_str = ex + ' ' + curr + ' ' + to_curr + ' ' + self.direction + ' ' + amount
        res = part.publish_event('admin_order', data=order_str)
        if res != 'success':
            content = Button(text='OK', size_hint=(1, .1))
            popup = Popup(title='Something went wrong', content=content, size_hint=(.5, .5))
            content.bind(on_press=popup.dismiss)
            popup.open()
        else:
            content = Button(text='OK', size_hint=(1, .1))
            popup = Popup(title='Success', content=content, size_hint=(.5, .5))
            content.bind(on_press=popup.dismiss)
            popup.open()
    ...
    def publish_stock(self):
        part = Particle_publish()
        print(part.auth())
        ex = self.ids['s_exchange'].text.upper()
        stock = self.ids['stock'].text.upper()
        amount = self.ids['amount'].text
        order_str = ex + ' ' + stock + ' ' + self.stock_dir + ' ' + amount
        res = part.publish_event('admin_order', data=order_str)
        if res != 'success':
            content = Button(text='OK', size_hint=(1, .1))
            popup = Popup(title='Something went wrong', content=content, size_hint=(.5, .5))
    ...
    def publish_all(self):
        part = Particle_publish()
        print(part.auth())
        ex = self.ids['all_ex'].text.upper()
        order_str = ex + ' ' + self.all_direc + 'ALL'
        res = part.publish_event('admin_order', data=order_str)
        if res != 'success':
            content = Button(text='OK', size_hint=(1, .1))
            popup = Popup(title='Something went wrong', content=content, size_hint=(.5, .5))
