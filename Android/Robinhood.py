...
class Robinhood:
    ...
    def login(self,
        ...
        if self.device_token == '':
            if os.path.exists('robin-token.txt'):
                with open('robin-token.txt', 'r') as f:
                    data = json.load(f)
                    self.device_token = data['device_token']
            else:
                self.device_token = self.generateDeviceToken()
                with open('robin-token.txt', 'w') as f:
        ...
        ## my token: '9256bb04-00e0-4220-8999-8229095fd1e5'

        # try:
        res = self.session.post(endpoints.login(), data=payload, timeout=15)
        data = res.json()
        try:
            self.response = data['challenge']['id']
        except Exception as e:
            print(e)
        if 'detail' in data:
            if data['detail'] == 'Request blocked, challenge issued.':
                return 'mfa required'
        elif 'access_token' in data.keys() and 'refresh_token' in data.keys():
            self.auth_token = data['access_token']
            self.refresh_token = data['refresh_token']
            # print("auth toke is {}".format(self.auth_token))
            # print("ref token is {}".format(self.refresh_token))
            self.headers['Authorization'] = 'Bearer ' + self.auth_token
            return True
        else:
            return False

    def mfa_login(self, sms_code):
        payload = {
        ...
        challenge_id = self.response
        self.headers["X-ROBINHOOD-CHALLENGE-RESPONSE-ID"] = challenge_id
        response_url = 'https://api.robinhood.com/challenge/{}/respond/'.format(challenge_id)
        response_payload = {'response': sms_code}
        try:
            res_chal = self.session.post(response_url, data=response_payload, timeout=15)
            res_final = self.session.post(endpoints.login(), data=payload, timeout=15)
        ...
        if 'access_token' in data_final.keys() and 'refresh_token' in data_final.keys():
            self.auth_token = data_final['access_token']
            self.refresh_token = data_final['refresh_token']
            # print("auth toke is {}".format(self.auth_token))
            # print("ref token is {}".format(self.refresh_token))
            self.headers['Authorization'] = 'Bearer ' + self.auth_token
            return True
